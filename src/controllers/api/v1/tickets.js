          ...
          async.waterfall(
              [
                  ...
                  function (status, user, done) {
                      ...
                      ticket.save(async function (err, t) {
                          if (err) return done({ status: 400, error: err });
      
                          t.populate("group owner priority", async function (err, tt) {
                              if (err) return done({ status: 400, error: err });
                              ...
                              const escalateTime = await settingSchema.getSettingByName("ticket:escalateTime", function (err, setting) {
                                if (err) return res.status(400).json({ success: false, error: err.message });
                            });

                            const delayTime = parseInt(escalateTime.value)
                              // **Schedule Escalation in 30 Minutes**
                             settingSchema.getSettingByName("escalateTicket:enable", async function (err, setting) {
