...
/**
 * Escalate Ticket by assigning it to an admin
 * @param {object} req Express Request
 * @param {object} res Express Response
 * @return {JSON} Response with success or error message
 */
ticketsV2.escalateTicket = function (req, res) {
  const ticketId = req.body.ticketId; 
  const socketId = req.body.socketId // Assuming ticketId is passed as a parameter
  const userSchema = require('../../../models/user'); // Import the user schema
  console.log(req.user, "hahhahha")
  if (!ticketId) {
    return res.status(400).json({ success: false, message: 'Ticket ID is required.' });
  }

  // Step 1: Find the ticket by ID
  Models.Ticket.getTicketById(ticketId, function (err, ticket) {
    if (err) {
      winston.warn(err);
      return res.status(500).json({ success: false, message: 'Error finding ticket.' });
    }
    ...
    // Step 2: Find the first admin user
    userSchema.getAdmins({ limit: 1, page: 0 }, function (err, admins) {
      if (err) {
        winston.warn(err);
        return res.status(500).json({ success: false, message: 'Error finding admin user.' });
      }
      ...
      const adminUser = admins[0]; // Get the first admin user

      // Step 3: Assign the ticket to the admin
      ticket.setAssignee(req.user._id, adminUser._id, function (err, updatedTicket) {
        if (err) {
          winston.warn(err);
          return res.status(500).json({ success: false, message: 'Error assigning ticket to admin.' });
        }

        // Step 4: Mark the ticket as escalated
        updatedTicket.escalated = true;
        updatedTicket.save(function (err, savedTicket) {
          if (err) {
            winston.warn(err);
            return res.status(500).json({ success: false, message: 'Error saving escalated ticket.' });
          }

           savedTicket.populate('group owner priority assignee', function (err, tt) {
                      if (err) return res.status(400).json({error: err}) 
                     console.log(tt, "ttt")
                      emitter.emit('ticket:escalated', {
                        hostname: req.headers.host,
                        socketId: socketId,
                        ticket: tt
                      })
          
                      return res.json({ success: true, message: 'Ticket escalated successfully.', ticket: savedTicket, assignee: adminUser  });
                    })
