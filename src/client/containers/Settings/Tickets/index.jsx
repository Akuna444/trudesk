...
class TicketsSettings extends React.Component {
  ...
  onEscalateTicketChange (e) {
    this.props.updateSetting({
      name: 'escalateTicket:enable',
      value: e.target.checked,
      stateName: 'escalateTicket',
      noSnackbar: false
    })
  }
  ...
  render () {
    ...
    console.log(this.props.settings.toJS(), "vprops")
...
<SettingItem
          title={'Enable Escalating Ticket'}
          subtitle={'Enable Escalating ticket when they are unanswered or need higher supervision'}
          tooltip={'If you disabled you cannot use escalating'}
          component={
            <EnableSwitch
              stateName={'escalateTicket'}
              label={'Enable'}
              checked={this.getSetting('escalateTicket')}
              onChange={e => {
                this.onEscalateTicketChange(e)
              }}
            />
          }
        />

{this.getSetting("escalateTicket") && (<SettingItem
          title={'Automatic Escalation Time'}
          subtitle={'Minimum minutes required to escalate the ticket to higher level'}
          component={
            <NumberWithSave
           
              stateName={'escalateTicketTime'}
              settingName={'ticket:escalateTime'}
              value={this.getSetting('escalateTicketTime')}
              width={'40%'}
            />
          }
        />)}
