...
function mailTemplates (callback) {
  ...
  var escalatedTicket = require('./json/mailer-escalated-ticket')
  var passwordReset = require('./json/mailer-password-reset')
  var templateSchema = require('../models/template')
  
  async.parallel(
    [ function (done) {
      templateSchema.findOne({ name: escalatedTicket.name }, function (err, templates) {
        if (err) return done(err)
        if (!templates || templates.length < 1) {
          return templateSchema.create(escalatedTicket, done)
        }
